...
function ClientBrand() {
  return (
    <img
      alt="ELIAN"
      className="h-7 w-auto max-w-28 border-l border-white/30 object-contain pl-4"
      src="/media/logos/elian.png"
    />
  )
}
  ...
  useGSAP(
    () => {
      ...
      const backgrounds = routeContent
        ...
        : []
      const fleetAgeDonuts = routeContent
        ? Array.from(routeContent.querySelectorAll<HTMLElement>('[data-fleet-age-donut]'))
        : []
      ...
      if (routeContent) {
        ...
        if (fleetAgeDonuts.length) {
          timeline.fromTo(
            fleetAgeDonuts,
            { autoAlpha: 0, rotate: -36, scale: 0.82 },
            {
              autoAlpha: 1,
              clearProps: 'transform,opacity,visibility',
              duration: 0.78,
